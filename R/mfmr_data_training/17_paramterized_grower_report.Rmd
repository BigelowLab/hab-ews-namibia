...
diatoms <- z |>
  ...
  mutate(`Cells/L` = round(`Cells/L`),
         threshold = c(NA, NA, params$pseudo_nitzshia_threshold, NA, NA, NA, NA),
...
dinos <- z |>
  ...
  mutate(`Cells/L` = round(`Cells/L`),
         threshold = c(params$prorocentrum_lima_threshold, NA, NA, NA, params$alexandrium_threshold),
         threshold_alert = c(NA, NA, NA, NA, "THRESHOLD EXCEEDED")) |>
  select(all_of(c("Specie", "Toxin producing species", "Cells/L", "threshold", "threshold_alert"))) |>
  rename("Species" = "Specie", 
         "Potential Toxin" = "Toxin producing species",
         "Threshold" = "threshold",
...
ggplot(diatoms[1:7,], aes(y=fct_reorder(Species, `Cells/L`), x=`Cells/L`)) + 
  geom_col() +
  ylab("Species") +
...
```{r echo=FALSE, fig.height=4}
ggplot(dinos[1:5,], aes(y=fct_reorder(Species, `Cells/L`), x=`Cells/L`, fill=`Threshold Alert`)) + 
  geom_col() + 
  guides(fill="none") +
  ylab("Species") +
